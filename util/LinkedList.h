...
// the capacity of the list, but the memory is never released. When the list is cleared,
// only its size is reset to zero, but no memory is released. It is reused for faster
// insertion of future elements instead. This way the capacity of the list only ever
...
public:
    ...
    // Returns true if all items in this and the other LinkList pass the == operator.
    ...
    // Returns true if NOT all items in this and the other LinkList pass the == operator.
    bool operator!=(const LinkedList<T>& other) const
    {
        return !(*this == other);
    }
    ...
    // to true with the given element d.
    ...
    // or at the tail if to was the last element.
