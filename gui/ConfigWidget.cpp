			...
			if (!parentFound)
			{
				...
				if (configKeyPart == configKeyPartList.last())
				{
					...
					slider->setPageStep(1);
                    			...
                    			slider->setValue(SLIDER_RANGE*config[configKey]/config.sliderRanges[configKey]);
					...
					QLabel* valueLabel = new QLabel(QString::number(config[configKey]).left(5));
					valueLabel->setMinimumWidth(37);
					valueLabel->setMaximumWidth(37);
...
void ConfigWidget::configSliderMoved(int idx)
{
	...
	QString key = config.memberNames[idx];
    	configLabels[idx]->setText(QString::number(config.sliderRanges[key]*(double)configSliders[idx]->value()/SLIDER_RANGE).left(5));
    	config[key] = config.sliderRanges[key]*(double)configSliders[idx]->value()/SLIDER_RANGE;
...
// e.g. config reset or the robot changed and a new config set was loaded.
void ConfigWidget::configChangedIn()
{
	...
	{
        	configSliders[idx]->setValue(SLIDER_RANGE*config[key]/config.sliderRanges[key]);
        	configLabels[idx]->setText(QString::number(config.sliderRanges[key]*(double)configSliders[idx]->value()/SLIDER_RANGE).left(5));
		idx++;
	}
...
void ConfigWidget::selectNextSlider()
{
	...
	while (parent != invisibleRootItem() and parent != 0)
	{
