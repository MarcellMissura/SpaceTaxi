...
public:
    ...
    void draw(double min=0, double max=1.0, double opacity=1.0, uint sliceIdx=0) const
    {
        ...
        for (uint j = 0; j < N[1]; j++)
        {
            for (uint i = 0; i < N[0]; i++)
            {
                ...
                QColor c = drawUtil.getHeightMapColor(Y[n], min, max);
        ...
        for (uint i = 0; i < N[0]; i++)
        {
            for (uint j = 0; j < N[1]; j++)
            {
                if (getDim() > 2)
                {
                    ...
                    if (counter > 0)
                    {
                        painter->setBrush(drawUtil.getHeatMapColor(counter, 0, N[2]));
    ...
    void drawCumulated() const
    {
        ...
        drawUtil.setHeatMapColor(QColor("AA00AA"));
        ...
        for (uint i = 0; i < N[0]; i++)
        {
            for (uint j = 0; j < N[1]; j++)
            {
                ...
                if (counter > 0)
                {
                    QColor c = drawUtil.getHeatMapColor(counter, 0, N[2]/10);
        ...
        drawUtil.setHeatMapColor(QColor("#11FF0000"));
