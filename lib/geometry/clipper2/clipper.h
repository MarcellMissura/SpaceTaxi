...
namespace Clipper2Lib {
  ...
  namespace details
  {
    ...
    inline void PolyPathToPolygonsD(const PolyPathD& polypath, LinkedList<Polygon>& polygons)
    {
        const PathD& path = polypath.Path();
        Polygon pol;
        for (uint i = 0; i < path.size(); i++)
            pol.appendVertex(path[i].x, path[i].y);
        polygons << pol;
        for (const auto& child : polypath)
            PolyPathToPolygonsD(*child, polygons);
    }
    ...
    inline bool PolyPath64ContainsChildren(const PolyPath64& pp)
    {
      for (const auto& child : pp)
      {
        ...
        int outsideCnt = 0;
        for (const Point64& pt : child->Path())
        {
          PointInPolygonResult result = PointInPolygon(pt, pp.Path());
  ...
  inline void PolyTreeToPolygonsD(const PolyTreeD& polytree, LinkedList<Polygon>& resultPolygons)
  {
    resultPolygons.clear();
    for (const auto& child : polytree)
      details::PolyPathToPolygonsD(*child, resultPolygons);
    return;
  }
