...
#include "lib/geometry/clipper2/clipper.engine.h"
...
namespace Clipper2Lib {
  ...
  bool BuildPolygonD(OutPt* op, bool reverse, Polygon& polygon, double inv_scale)
  {
    if (!op || op->next == op)
      return false;
    ...
    polygon.appendVertex(lastPt.x * inv_scale, lastPt.y * inv_scale);
    ...
    if (polygon.size() == 3 && IsVerySmallTriangle(*op2)) return false;
    return true;
  }
  ...
  void ClipperD::BuildPolygonTreeD(PolyPathD& polytree)
  {
        polytree.Clear();
