...
namespace Clipper2Lib {
	...
	public:
        ...
        const Path64& Path() const { return path_; }

		double Area() const
		{
			return std::accumulate(childs_.cbegin(), childs_.cend(),
                Clipper2Lib::Area<int64_t>(path_),
	...
	public:
        ...
        double InvScale() const { return inv_scale_; }

        void Clear() override { childs_.resize(0); }

        size_t Count() const override {	return childs_.size(); }

        const PathD& Path() const { return path_; }

		double Area() const
		{
			return std::accumulate(childs_.begin(), childs_.end(),
                Clipper2Lib::Area<double>(path_),
				[](double a, const auto& child) {return a + child->Area(); });
		}

        PolyPath* AddChild(const Path64& path)
        {
            std::unique_ptr<PolyPathD> up = std::make_unique<PolyPathD>();
            childs_.resize(childs_.size()+1);
            childs_.back() = std::move(up);
            PolyPathD* pp = childs_.back().get();
            pp->SetInvScale(inv_scale_);
            int error_code = 0;
            pp->path_ = ScalePath<double, int64_t>(path, inv_scale_, error_code);
            pp->parent_ = this;
            return pp;
        }
	};
	...
	private:
        ...
        void BuildPolygonsD(LinkedList<Polygon>& resultPolygons);
		void BuildTreeD(PolyPathD& polytree, PathsD& open_paths);
        void BuildPolygonTreeD(PolyPathD& polytree);
	public:
        ...
        bool Execute(ClipType clip_type, FillRule fill_rule, LinkedList<Polygon>& resultPolygons)
        {
            if (ExecuteInternal(clip_type, fill_rule, false))
                BuildPolygonsD(resultPolygons);
            CleanUp();
            return succeeded_;
        }
