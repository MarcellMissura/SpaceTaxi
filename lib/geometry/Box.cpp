...
// point and the distances to the top, left, bottom, right borders relative
// to the reference point.
Box::Box(Vec2 p, double t, double l, double b, double r)
{
    this->x = p.x;
    this->y = p.y;
    this->t = t;
...
// point and the distances to the top, left, bottom, right borders relative
// to the reference point.
void Box::set(Vec2 p, double t, double l, double b, double r)
{
    this->x = p.x;
    this->y = p.y;
    this->t = t;
...
// This is an approximate but very fast test. For a more precise test, use the intersection(Line) function.
bool Box::intersects(const Line &l, bool debug) const
{
    if (debug)
        qDebug() << "Box::intersects: id" << l.id << l.left() << l.right() << l.top() << l.bottom()
                 << "|" << left() << right() << top() << bottom()
                 << "|" << (l.left() > right()) << (l.right() < left()) << (l.bottom() > top()) << (l.top() < bottom());
...
// Determine the intersection of the path with this box.
...
// the returned Pose.
Pose2D Box::intersection(const Vector<Vec2>& path) const
{
    Pose2D intersectionPoint;
    bool targetFound = false;
    uint i = 1;
    while (!targetFound && i < path.size())
    {
        // If the current path node is inside the box...
        if (intersects(path[i]))
        {
            intersectionPoint.setPos(path[i]);
            intersectionPoint.setHeading((path[i]-path[i-1]).angle());
        }
        ...
        {
            targetFound = true;
            intersectionPoint.setPos(intersection(Line(path[i], path[i-1]))); // We can be sure there is only one intersection.
            intersectionPoint.setHeading((path[i]-path[i-1]).angle());
        }
