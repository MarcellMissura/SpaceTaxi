...
const Vector<Vec2>& DijkstraMap::dijkstraPath(const Vec2 &start)
{
    if (debug > 0)
        qDebug() << "DijkstraMap::dijkstraPath(const Vec2 &start):" << start << Vec2u(getNodeIndex(start));

    //qDebug() << "compute path for:" << start << Vec2u(getNodeIndex(start));
...
// Returns the path found by dijkstraPath().
const Vector<Vec2>& DijkstraMap::getPath() const
{
    return path;
}
...
void DijkstraMap::draw() const
{
    ...
    for (uint i = 0; i < N.x; i++)
    {
        for (uint j = 0; j < N.y; j++)
        {
            if (nodes[i][j].h > 0)
            {
                QColor c = colorUtil.getHeightMapColor(nodes[i][j].h, 0, max);
...
void DijkstraMap::draw(QPainter* painter) const
{
    ...
    for (uint i = 0; i < N.x; i++)
    {
        for (uint j = 0; j < N.y; j++)
        {
            if (nodes[i][j].h > 0)
            {
                QColor c = colorUtil.getHeightMapColor(max-nodes[i][j].h, 0, max);
...
void DijkstraMap::drawPath(QPainter* painter) const
{
    if (!path.isEmpty())
    {
        ...
        double s = 0.03;
        painter->strokePath(p, colorUtil.penRedThick);
        painter->setBrush(colorUtil.brushOrange);
        painter->setPen(colorUtil.penOrange);
