...
// Returns the depth of the footstep in the search tree.
uint UnicycleSearchNode::getDepth()
{
    return depth;
}

// Creates a search node from the given node fs using the action acc for time dt.
void UnicycleSearchNode::propagate(const UnicycleSearchNode* u, const Vec2& acc)
{
    *this = *u;
    parent = u;
